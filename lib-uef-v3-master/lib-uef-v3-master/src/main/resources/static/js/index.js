...
if (typeof profilePopupHandler === 'undefined') {
    const profilePopupHandler = true;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if(editProfileBtn && editProfileModalEl) {
            ...
            saveProfileChangesBtn.addEventListener('click', async function() {
                ...
                // 2. Kiểm tra dữ liệu và thu thập lỗi
                let errors = [];
                ...
                if (!email.includes('@')) {
                    errors.push("Định dạng email không hợp lệ, phải có ký tự @.");
                }

                // 3. Hiển thị lỗi hoặc gửi đi
                if (errors.length > 0) {
                    // Nếu có lỗi, hiển thị tất cả lỗi trong một thông báo và dừng lại
                    alert("Vui lòng sửa các lỗi sau:\n\n- " + errors.join("\n- "));
                    return; // Dừng, không gửi form đi
                }

                // --- KẾT THÚC PHẦN VALIDATION ---


                // Nếu không có lỗi, tiếp tục với logic gửi dữ liệu đi như cũ
                const formData = {
                    fullName: fullName,
                    phone: phone,
                    email: email, // Email không được sửa nhưng vẫn cần cho các bước sau này
                ...
                try {
                    const response = await fetch('/reader/api/profile/update', {
                        method: 'POST',
                    ...
                    if(response.ok) {
                        ...
                        if (nameSpan) {
                            nameSpan.textContent = result.newFullName;
                        }
